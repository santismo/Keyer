...
assert.match(html, /id="keyboardToneMode"[\s\S]*?<option value="midi">MIDI notes<\/option>/);
assert.match(html, /id="fretboardToneMode"[\s\S]*?<option value="midi">MIDI notes<\/option>/);
assert.match(desktopHtml, /id="keyboardToneMode"[\s\S]*?<option value="midi">MIDI notes<\/option>/);
assert.match(desktopHtml, /id="fretboardToneMode"[\s\S]*?<option value="midi">MIDI notes<\/option>/);
...
assert.match(html, /id="guitarVoicingStyle"[\s\S]*?<option value="original-midi">Original MIDI<\/option>/);
assert.match(desktopHtml, /id="guitarVoicingStyle"[\s\S]*?<option value="original-midi">Original MIDI<\/option>/);
...
assert.match(js, /function centerMidiRollPlayhead\(/);
assert.match(js, /function centerMidiRollPlayhead\(scroll, playhead\)/);
assert.match(js, /playhead\.getBoundingClientRect\(\)/);
assert.match(js, /svg\.setAttribute\('preserveAspectRatio', 'none'\)/);
assert.match(js, /function keyboardMidiDisplayActive\(/);
assert.match(js, /function fretboardMidiDisplayActive\(/);
assert.match(js, /function rawMidiVisualizationActive\(/);
assert.match(js, /function scheduleRawMidiVisualsForSegment\(/);
assert.match(js, /\['scale', 'chord', 'voicing', 'midi', 'none'\]\.includes\(mode\)/);
...
assert.match(css, /\.midi-roll svg\s*\{[^}]*width:\s*max\(100%, var\(--midi-roll-width, 760px\)\)/);
assert.match(css, /\.piano-key\.white\.raw-midi-tone/);
assert.match(css, /\.fretboard-cell\.raw-midi-tone/);
assert.match(css, /\.piano-key\.white\.raw-midi-tone\s*\{[^}]*background:\s*#45f58c/);
...
assert.match(js, /function originalMidiModeSelected\(/);
assert.match(js, /function originalMidiNotes\(/);
assert.match(js, /state\.guitarVoicingStyle === 'original-midi'/);
assert.match(sourceForFunction('originalMidiModeSelected'), /state\.guitarVoicingStyle === 'original-midi'/);
assert.match(sourceForFunction('fretboardMidiDisplayActive'), /state\.guitarVoicingStyle === 'original-midi'/);
assert.match(sourceForFunction('currentChordPlayback'), /if \(originalMidiModeSelected\(\)\) return \{ voicing: \[\], visual: 'chord' \};/);
assert.match(sourceForFunction('streamChordVoicingForEvent'), /originalMidiModeSelected\(\)/);
...
assert.match(sourceForFunction('activeOriginalMidiNotes'), /rawMidiNotesAtBeat\(state\.playheadBeat, notes\)/);
assert.match(sourceForFunction('seekTimelineBeat'), /selectEvent\(entry\.eventIndex, false\);[\s\S]*state\.playheadBeat = targetBeat;/);
