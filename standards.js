(() => {
  ...
  function syncMidiSourceStatus() {
    ...
    if (state.midi) {
      ...
      const caution = !state.midiChart && !['parkerize', 'tab-file'].includes(state.midiEntry?.type) ? ' · check the form matches' : '';
  ...
  function applyLoadedSong(song, { transport = false, preloadedMidi = null } = {}) {
    ...
    state.irealChart = createChartData('ireal', bars, song.playbackOrder, {
      sourceKey: song.key,
      tempoBpm: song.bpm,
      explicitTiming: Boolean(song.parkerXmlTiming || song.tabTiming)
    });
    if (!state.irealChart.events.length) return false;
    clearMidiSource();
    const generatedPractice = parkerizeActive();
    state.preferSoloChorus = generatedPractice || Boolean(song.tabSource) || ['solos', 'parker'].includes(state.songAvailabilityFilter);
  ...
  function matchingSongs(query) {
    ...
    const bank = state.songAvailabilityFilter === 'tab-files'
      ...
      : state.songAvailabilityFilter === 'favorites'
        ...
        : state.songs;
    const source = q
      ? bank.filter(song => `${song.title} ${song.composer} ${song.style} ${song.azMidiEntry?.file || ''} ${song.tabCatalogEntry?.file || ''}`.toLowerCase().includes(q))
      : bank;
  ...
  function hideSearchResults() {
    ...
    if (state.songs.length) {
      ...
      elements.libraryStatus.textContent = `${state.songs.length.toLocaleString()} jazz-standard charts · ${multiChorusCount} multi-chorus studies · ${parkerCount} Parker solos · ${azCount.toLocaleString()} A–Z MIDI songs · ${tabCount.toLocaleString()} tab files`;
    }
  }
  ...
  function startVoice(id, midi, duration = null, displayMidi = midi, visual = 'all', authoredPosition = null) {
    ...
    color.start(now);
    const voice = { midi, displayMidi, visual, authoredPosition, envelope, oscillators: [oscillator, color], startedAt: now, timerId: null };
    voices.set(id, voice);
    if (duration) {
      voice.timerId = window.setTimeout(() => {
        if (voices.get(id) === voice) stopVoice(id);
  ...
  elements.songAvailabilityFilter?.addEventListener('change', () => {
    state.songAvailabilityFilter = ['favorites', 'melody', 'chords', 'solos', 'parker', 'az-midi', 'tab-files', 'parkerize'].includes(elements.songAvailabilityFilter.value)
